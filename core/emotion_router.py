...
class EmotionRouter:
    ...
    def _predict(self, text: str) -> Tuple[str, List[Tuple[str, float]]]:
        ...
        for emotion, base_conf, patterns in _RULES:
            if not patterns:
                continue
    ...
    def _build_signal(self, primary: str, labels: list) -> EmotionSignal:
        eye_expr = _REACTION_MAP.get(primary, "neutral")
        hint     = _HINTS.get(primary, "Respond clearly and warmly.")
        return EmotionSignal(
            labels=labels,
            primary=primary,
    ...
    def _neutral_signal(self) -> EmotionSignal:
        return EmotionSignal(
            labels=[("neutral", 1.0)],
            primary="neutral",
            eye_expression="neutral",
